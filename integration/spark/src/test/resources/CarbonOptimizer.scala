...
import org.apache.carbondata.spark.{CarbonFilters, CarbonSorts}
...
/*        case sort: Sort if !sort.child.isInstanceOf[CarbonDictionaryTempDecoder] =>
          ...
          case limit: Limit if !decoder && limit.child.isInstanceOf[Sort] =>
           ...
           var newLimit = Limit(limit.limitExpr, child)
          val attrsOnSort = new util.HashSet[AttributeReferenceWrapper]()
          sort.order.map { s =>
            s.collect {
              case attr: AttributeReference
          ...
          if (attrsOnSort.size() == 1) { //&& !child.isInstanceOf[Sort]) {
            ...
            if(sortByDimFlg){
               ...
               child = CarbonDictionaryTempDecoder(new util.HashSet[AttributeReferenceWrapper](), new util.HashSet[AttributeReferenceWrapper](), limit)
        ...
        case sort: Sort if !decoder && !sort.child.isInstanceOf[CarbonDictionaryTempDecoder] =>
          ...
          if (attrsOnSort.size() > 0) { //&& !child.isInstanceOf[Sort]) {
            ...
            if(attrsOnSort.size() == 1 && sortByDimFlg){
               ...
               child = CarbonDictionaryTempDecoder(new util.HashSet[AttributeReferenceWrapper](), new util.HashSet[AttributeReferenceWrapper](), sort)
            }else{
