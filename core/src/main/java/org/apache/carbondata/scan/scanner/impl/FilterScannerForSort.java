...
import org.apache.carbondata.scan.result.impl.FilterQueryScannedResultForSort;
 ...
 */
public class FilterScannerForSort extends AbstractBlockletScanner {
  ...
  public FilterScannerForSort(BlockExecutionInfo blockExecutionInfo,
                       QueryStatisticsModel queryStatisticsModel) {
    super(blockExecutionInfo);
    scannedResult = new FilterQueryScannedResultForSort(blockExecutionInfo);
...
public boolean applyFilter(BlocksChunkHolder blocksChunkHolder) throws QueryExecutionException {
  ...
  @Override public AbstractScannedResult scanBlockletForSort(BlocksChunkHolder blocksChunkHolder, SortOrderType orderType)
      throws QueryExecutionException {
    try {
    	fillScannedResultForSort(blocksChunkHolder, orderType);
    } catch (FilterUnsupportedException e) {
      throw new QueryExecutionException(e.getMessage());
    }
    return scannedResult;
  ...
  private void fillScannedResult(BlocksChunkHolder blocksChunkHolder)
	...
	//System.out.println("fillScannedResult: "+(end -start));
    //TODO
  ...
  private void fillScannedResultForSort(BlocksChunkHolder blocksChunkHolder, SortOrderType orderType)
	      throws FilterUnsupportedException {
...
/*	    //TODO
	    ...
	    FileHolder fileReader = blocksChunkHolder.getFileReader();
	    int[] allSortDimensionBlocksIndexes =
	    		blockExecutionInfo.getAllSortDimensionBlocksIndexes();
	    DimensionColumnDataChunk[] dimensionColumnDataChunks =  blocksChunkHolder.getDimensionDataChunk();
	    		//new DimensionColumnDataChunk[blockExecutionInfo.getTotalNumberDimensionBlock()];
	    // read dimension chunk blocks from file which is not present
	    for (int i = 0; i < allSortDimensionBlocksIndexes.length; i++) {
	      if (null == blocksChunkHolder.getDimensionDataChunk()[allSortDimensionBlocksIndexes[i]]) {
	        dimensionColumnDataChunks[allSortDimensionBlocksIndexes[i]] =
	            blocksChunkHolder.getDataBlock()
	                .getDimensionChunk(fileReader, allSortDimensionBlocksIndexes[i], blocksChunkHolder.getLimit());
	      }/* else {
	        dimensionColumnDataChunks[allSortDimensionBlocksIndexes[i]] =
	            blocksChunkHolder.getDimensionDataChunk()[allSortDimensionBlocksIndexes[i]];
	      }*/
	    }
	    //scannedResult.setMaxLogicalRowIdByLimit(dimensionColumnDataChunks[blockExecutionInfo.getAllSortDimensionBlocksIndexes()[0]].getAttributes().getInvertedIndexesReverse().length);
