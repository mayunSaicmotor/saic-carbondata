...
public class SnappyNew
{
    ...
    public static byte[] rawCompress(Object data, int byteSize)
            throws IOException
    {
        byte[] buf = new byte[SnappyNew.maxCompressedLength(byteSize)];
    ...
    public static byte[] uncompress(byte[] input)
            throws IOException
    {
        byte[] result = new byte[SnappyNew.uncompressedLength(input)];
        SnappyNew.uncompress(input, 0, input.length, result, 0);
        return result;
    }
    
    public static byte[] uncompress(byte[] input, int limit)
            throws IOException
    {
    	int tmp = SnappyNew.uncompressedLength(input);
    	if(limit>tmp){
    		limit = tmp;
    	}
    ...
    public static char[] uncompressCharArray(byte[] input, int offset, int length)
            throws IOException
    {
        int uncompressedLength = SnappyNew.uncompressedLength(input, offset, length);
    ...
    public static double[] uncompressDoubleArray(byte[] input)
            throws IOException
    {
        int uncompressedLength = SnappyNew.uncompressedLength(input, 0, input.length);
     ...
     * @throws IOException
     */
    public static long[] uncompressLongArray(byte[] input, int limit)
            throws IOException
    {
        return uncompressLongArray(input, 0, input.length, limit);
    }
     ...
     * @throws IOException
     */
    public static long[] uncompressLongArray(byte[] input, int offset, int length, int limit)
            throws IOException
    {
    	if(limit<=0){
    		limit =  SnappyNew.uncompressedLength(input, offset, length);
    	}
        //int uncompressedLength = SnappyNew.uncompressedLength(input, offset, length);
